...
veryOldInventors = inventors.filter(filterInventors);
...
const mergeName = (inventor) => {
    return {first:`${inventor.first}` , last:`${inventor.last}`};
} 

inventorNames = inventors.map(mergeName);
...
sortedByBirthYear = inventors.sort((inventor1 , inventor2) => inventor1.year - inventor2.year);
...
isAdultPresent = devs.some(checkAge);
...
const checkAgeLargerThan19 = (dev) => {
    return (2024 - dev.year) > 19;
}
isEveryone19OrOlder = devs.every(checkAgeLargerThan19);
...
commentById = comments.find(findComment);
...
const findCommentIdx = (comment) => {
    return comment.id === 123523
}

idx = comments.findIndex(findCommentIdx);
